...
def reset_game():
    global lives, score, mystery_timer
    ...
    lives = 3
    spawn_aliens(6,8)
    mystery_timer = pygame.time.get_ticks()/1000
    mystery_rect.topleft = (650,80)
...
def display_lives():
    ...
    for live in range(lives - 1):
        x = life_x_start_pos + (live * (life_image_width + 10))
        window.blit(life_image,(x,8))
        
def move_mystery():
    global mystery_timer, display_mystery
    if not won:
        if pygame.time.get_ticks()/1000 - mystery_timer > mystery_spawn_time:
...
def collide_with_mystery():
    global score, display_mystery, mystery_timer
...
def collide_with_player():
    global lives, game_over, display_mystery
    for alien_bullet_rect in alien_bullets[:]:
       if alien_bullet_rect.colliderect(player_rect) and not won:
